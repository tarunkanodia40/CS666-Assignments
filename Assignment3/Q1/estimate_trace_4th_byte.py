import csv
import numpy as np
import matplotlib.pyplot as plt

wstart = 2
wstop = 13
wlen=11

InvSbox = (
    ,
)

myfile = "HW_power_trace_1.csv"

sarr_a = np.array([0]*wlen)
sarr_b = np.array([0]*wlen)

ntraces_a = 0
ntraces_b = 0

dom_arr = np.zeros((256,wlen),dtype='float')

number_of_traces = 10000
for kb in range(256):      
        print("Processing: "+hex(kb))
        dofmean=np.zeros(wlen,dtype='float')
        with open(myfile) as csv_file:
            i=0
            csv_reader = csv.reader(csv_file, delimiter=',')
            for row in csv_reader:
                ct = int(float(row[1]))
                # 4th byte
                ct_temp=(ct>>88)&255
                state_9th=InvSbox[ct_temp^kb]
                binexp = '{0:08b}'.format(state_9th)
            
                if(binexp[0] == '1'):
                    sarr_a = np.add(sarr_a,np.array(row[wstart:wstop]).astype(float))
                    ntraces_a = ntraces_a + 1
                if(binexp[0] == '0'): 
                    sarr_b = np.add(sarr_b,np.array(row[wstart:wstop]).astype(float))
                    ntraces_b = ntraces_b + 1
                i=i+1
                if i==number_of_traces: break
                
        marr_a = sarr_a/float(ntraces_a)
        marr_b = sarr_b/float(ntraces_b)

        dofmean = np.subtract(marr_a,marr_b)
        dofmean = np.abs(dofmean)
        
        dom_arr[kb] = dofmean
        print(max(dofmean))
        ntraces_a=0
        ntraces_b=0
        sarr_a = np.array([0]*wlen)
        sarr_b = np.array([0]*wlen)
        del dofmean

fig, ax1 = plt.subplots()
maxval=0
for i in range(256):
    row = dom_arr[i]
    tp = range(len(row))
    if(maxval<max(row)):
        maxval=max(row)
        correct_key=i
        correct_row=row
        
print("correct_fourth_key_byte="+str(correct_key))

for i in range(256):
    row=dom_arr[i]
    tp=range(len(row))
    if (i==correct_key):
        plt.plot(range(len(correct_row)),correct_row , 'r', linewidth=0.2,label='Correct Fourth Key Byte')
    else:
        plt.plot(tp, row, 'k', linewidth=0.2)
        
ax1.legend()
plt.locator_params(axis='y', nbins=5)
plt.title('Difference of Mean Plot')
plt.xlabel('Sample Points')
plt.ylabel('Difference of Mean')
plt.savefig("DOM_FourthKeyByte.png",dpi=1200,bbox_inches='tight')
plt.show()
